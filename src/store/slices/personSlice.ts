...
export const fetchPersonById = createAsyncThunk<Person, { id: number }, { rejectValue: string }>(
    'person/fetchPersonById',
    async ({ id }, { rejectWithValue }) => {
        try {
            const res = await axios.get<Person>(`${API_URL}/person/${id}?language=en-US`, options);
            if (res.status !== 200) throw new Error('Failed to fetch person');
            return res.data;
        } catch (err) {
            const error = err as AxiosError<{ status_message?: string }>;
